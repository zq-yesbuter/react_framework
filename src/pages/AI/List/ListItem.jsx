...
const { Search } = Input;
function ListItem({ dispatch,onSelectItem,headImage,selectId,id,name,bespeakTime=new Date(),lastConnectTime=new Date(),onChange,content }) {
  ...
  return (
    <li className={styles[`chanceItem_${itemStyle}`]}>
      ...
      <div className={styles.item} onClick={() => onSelectItem(id)}>
        ...
        <div title="导入时间">
            {moment(lastConnectTime).format("YY-MM-DD HH:mm")}
        </div>
        </div>
      </div>
